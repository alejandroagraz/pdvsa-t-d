var addthis_config = {
  ...
  ui_language: ($("html").attr("lang")=="en")?"us":"esm"
};

var MedlinePlus = (function _mp(undefined) {
  var protocol = window.location.protocol;
  var _root = $("html").attr("data-root");
  var _taggingURL = "//wtsdc.nlm.nih.gov/dcsutpzmv10000gkm7ykazpb0_2x3w/njs.gif?dcsuri=/nojavascript&WT.js=No&WT.tv=9.4.0&dcssip=www.nlm.nih.gov&dcsid=dcsutpzmv10000gkm7ykazpb0_2x3w"+"&token=" + encodeURIComponent(rand(1,100000));
  var _pcss = {};
  var js1 = [];
  var acnjs = [
  ...
  var foresee = {
         "en":(protocol+"//www.nlm.nih.gov/medlineplus/jslib/foresee2/foresee-trigger.js"),
         "es":(protocol+"//www.nlm.nih.gov/medlineplus/jslib/foresee2/foresee-trigger.js")};
  var addthisJS = [protocol+"//s7.addthis.com/js/300/addthis_widget.js#domready=1"];
  ...
  $(document).ready(function _IIFE() {
    ...
    if($(".addthis_toolbox").length > 0) {
      getScriptCached(addthisJS, function(){});
    }
    ...
    // http://www.nlm.nih.gov/core/jquery/1.4/jquery.min.js is loaded.
    if (bmapjs[_pid]) {
  ...
  function init() {
    if (top.location != self.location && notTouchCnt && notTouchCnt2 ) {
      if ((!/\/spanish\//.test(self.location)) && (!/project=medlineplus-spanish/.test(self.location))) {
        location.replace(protocol+'//www.nlm.nih.gov/medlineplus/framing.html?rf=' + self.location);
      }
      else {
        location.replace(protocol+'//www.nlm.nih.gov/medlineplus/spanish/framing.html?rf=' + self.location);
      }
    }
